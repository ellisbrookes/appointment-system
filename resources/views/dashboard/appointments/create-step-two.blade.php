  ...
  <div class="flex flex-col justify-center mx-auto w-full max-w-5xl p-6">
    ...
    <form action="{{ route('dashboard.appointments.create.step.two.post') }}" method="POST" class="flex flex-col space-y-4 w-full">
      ...
      <div class="text-center flex justify-center space-x-2">
        <a href="{{ route('dashboard.appointments.create.step.one') }}" class="bg-gray-600 text-white py-3 px-4 rounded-md hover:bg-gray-700 focus:outline-hidden focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50 transition duration-200">Previous Step</a>
        
        <button type="submit" class="bg-blue-600 text-white py-3 px-4 rounded-md hover:bg-blue-700 focus:outline-hidden focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-200">Next Step</button>
      </div>
    </form>
  </div>
