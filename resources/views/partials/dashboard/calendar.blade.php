<div class="bg-white p-8 rounded-lg shadow-xl w-full h-full flex flex-col justify-between">
    ...
    @for($day = 1; $day <= $daysInMonth; $day++)
      <form method="GET" action="{{ route('appointments.view') }}" class="w-full h-full">
        ...
        <div class="relative p-4 h-auto rounded-lg flex items-center justify-center text-lg font-medium
          ...
          <button type="submit" class="absolute inset-0 w-full h-full"></button>

          <!-- Day number (on top of the button) -->
  ...
  <div class="mt-8 w-full flex justify-around">
    ...
    <div class="flex items-center space-x-2 bg-yellow-100 text-yellow-600 rounded-lg px-4 py-2 shadow-md cursor-pointer hover:bg-yellow-200">
      <span class="text-sm font-semibold">Blocked</span>
    </div>
    <div class="flex items-center space-x-2 bg-red-100 text-red-600 rounded-lg px-4 py-2 shadow-md cursor-pointer hover:bg-red-200">
